...
class ModelAdminReorder(MiddlewareMixin):
    ...
    def init_config(self, request, app_list):
        ...
        self.config_index = getattr(settings, 'ADMIN_REORDER_INDEX', None)
        if not self.config:
    ...
    def get_app_list(self, url_name):
        ordered_app_list = []
        for app_config in self.config:
            app = self.make_app(app_config)
            if app:
                if url_name == 'index' and app['app_label'] in self.config_index:
                    ordered_app_list.append(app)
                elif url_name == 'app_list':
                    ordered_app_list.append(app)
    ...
    def process_template_response(self, request, response):
        ...
        try:
            ...
            ordered_app_list = self.get_app_list(url.url_name)
        ...
        try:
            ...
            ordered_all_list = self.get_app_list('app_list')
            response.context_data['available_apps'] = ordered_all_list
        except KeyError:
            # there is no app_list! nothing to reorder
            pass
